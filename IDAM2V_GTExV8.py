...
def read_normalized_series_file(series_file, take_median=True):
    ...
    # num_of_tissue = 31
    for key, vals in expression_dict.items():
        new_vals = []
        num_dup = len(vals)
...
def get_mean_expression_for_tissue_multiple_sampels(samples, expression_dict, use_mean=False, log2=True):
    ...
    aver_expr_vals = {}
    for key, vallist in expression_dict.items():
    ...
    new_aver_expr_vals = {}
    for key, val in aver_expr_vals.items():
        # if max(val) < CUTOFF:
        #    continue
...
# 读取GTEx数据集
def read_gtex_expression(RNAseq_file='data/gtex/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct.gz',
...
def read_evolutionary_expression_data(input_file='data/GSE43520/genes.fpkm_table', use_mean=False, log2=True):
    ...
    fp.close()
    # pdb.set_trace()
    data_dict, sample_set = get_mean_expression_for_tissue_multiple_sampels(samples, data_dict, use_mean=use_mean,
                                                                            log2=log2)
...
def read_average_read_to_normalized_RPKM(input_file='data/GSE30352/genes.fpkm_table', readlength=75, use_mean=False,
    ...
    # pdb.set_trace()
    final_vals, sample_set = get_mean_expression_for_tissue_multiple_sampels(samples, express_vals, use_mean=use_mean,
                                                                             log2=log2)
...
def read_average_read_to_normalized_RPKM_old(input_file, readlength=75, use_mean=False, log2=True):
    ...
    for x in range(total_exp_vals.shape[0]):
        final_vals[gene_list[x]] = []
        for y in range(total_exp_vals.shape[1]):
    ...
    final_vals, sample_set = get_mean_expression_for_tissue_multiple_sampels(samples, final_vals, use_mean=use_mean,
                                                                             log2=log2)
...
def read_gencode_gene_type():
    ...
    gene_name_ensg = {}
    gene_id_position = {}
    fp = gzip.open('data/gencode.v19.genes.v6p_model.patched_contigs.gtf.gz', 'rb')
    for line in fp:
        # print('[Debug]line:', line)
        if b'#' in line:
            continue
        values = line.decode().rstrip('\r\n').split('\t')
        # windows10下无法读取gencode.v19.genes.v6p_model.patched_contigs.gtf.gz文件
...
def plot_roc_curve_miRNA(labels, probality, legend_text):
    ...
    # print('line1133',labels,probality )
...
def get_counts_key(data):
    ...
    # for val, ind in key_count.items():
...
def read_mirna_disease(mirna_dis_file='data/alldata-disease.txt'):
    mirna_dis = {}
    head = True
    with open(mirna_dis_file, 'r', encoding='UTF-8') as fp:
        for line in fp:
            if head:
                head = False
                continue
...
def read_gencode_mirna_annotation_new():
    ...
    for line in fp:
        if b'#' in line:
            continue
        values = line.decode().rstrip('\r\n').split('\t')
        # print(sys._getframe().f_lineno, '[line1236]:', values)
        if values[2] != 'gene':
            continue
        gene_ann = values[-1]
        chrm = 'chr' + values[0]
...
def read_expression_data(input_file, data=3):
    ...
    for key, val in rain_interaction.items():
        mir, eng = key
    ...
    new_mirna_disease = {}
    # print('1494:', mirna_disease)
    for mirna, val in mirna_disease.items():
...
def run_gcn():
    ...
    # print(sys._getframe().f_lineno, adj.shape)
    # print(sys._getframe().f_lineno, features.shape)
    # print(sys._getframe().f_lineno, labels.shape)
    # print(sys._getframe().f_lineno, idx_train.shape)
